...
def ex_rage():
    ...
    currency_name_dict={'미국 달러':'USD','유럽 연합 유로':'EUR','일본 엔화':'JPY'}
    ...
    if clicked:
        ...
        df_exchge = get_exchange(currency_code)
        # print(df_exchge)
        
        # 원하는 열만 선택
        df_exchge_rate = df_exchge[['날짜','매매기준율','사실 때','파실 때','보내실 때','받으실 때']]
        df_exchge_rate2 = df_exchge_rate.set_index('날짜')

        # 날짜열의 데이터 변경
        df_exchge_rate2.index = pd.to_datetime(df_exchge_rate2.index,format='%Y-%m-%d',errors="ignore")
        

        #환율 데이터 표시
        st.subheader(f"{currency_name} 환율 데이터")
        st.dataframe(df_exchge_rate2.head(20))

        # 차트(선 그래프) 그리기
        matplotlib.rcParams['font.family']='Malgun Gothic'
        
        ax = df_exchge_rate2['매매기준율'].plot(figsize=(15,5),grid=True)
        ...
        csv_data = df_exchge_rate.to_csv()

        #dataframe 데이터를 엑셀excel 데이터 변환
        excel_data = BytesIO()  #메모리 버퍼(임시장소)에 바이너리 객체 생성
        df_exchge_rate.to_excel(excel_data)  # 엑셀 형식으로 버퍼에 쓰겠다 라는 의미
